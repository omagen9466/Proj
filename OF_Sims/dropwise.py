...
def Condense(T_inf= 373,T_wall=363,total_pressure=101325,delta_t=10**(-1),theta=86,endtime=1,step=1e-6,accom=0.02,lat_heat=2429.8*1e3):
    ...
    # F = lambda t, r: (((T_inf-T_wall)/(960*lat_heat*(2-3*np.cos(theta*np.pi/180)+np.cos(theta*np.pi/180)**(3))))*(1-(var_dict['r_min']/r)))/(((theta*np.pi/180)/(4*var_dict['k_copp']*np.sin(theta*np.pi/180)**(2)))*r+(1/(2*var_dict['h_int']*(1-np.cos(theta*np.pi/180)))))
